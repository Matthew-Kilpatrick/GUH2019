<?php
    ...
    foreach ($vehicles as $vehicle) {
        if (getLocation($vehicle['next_location_id'])['enabled'] == True) {
    ...
    $tripIDs = [];
    $vehicle = getVehicle($minVehicle);
    if ($vehicleData['time_to_customer'] > 0) {
        // Trip needed to reach customer
        $data = [
            'start_location_id' => $vehicle['next_location_id'],
            'end_location_id' => $startLocation,
    ...
    $tripIDs[] = $_DB->insert('trips', $data);
    // UPDATE CHARGE & NEXT LOCATION OF VEHICLE
    ...
    if ($minTime != 0) {
        echo json_encode(['message' => 'The vehicle will be ready at location in ' . round($minTime) . ' minutes', 'ids' => implode($tripIDs, ',')]);
    } else {
        echo json_encode(['message' => 'The vehicle is ready at the start location', 'ids' => implode($tripIDs, ',')]);
    }
?>